...
if not any(cell['cell_type']=='markdown' and 'curva de oferta' in normalize(''.join(cell.get('source', []))) for cell in cells):
    supply_md = {
        'cell_type': 'markdown',
        'metadata': {},
    ...
    calib_md_idx = next(i for i, cell in enumerate(cells) if cell['cell_type']=='markdown' and 'calibracao heuristica' in normalize(''.join(cell.get('source', []))))
...
# Baseline display
base_cell = find_cell("baseline_cfg['params']['nu']")
